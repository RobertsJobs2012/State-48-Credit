      ...
      <div className="max-w-4xl mx-auto px-6 relative z-10">
        ...
        <div className="relative min-h-[300px] md:min-h-[240px] flex items-center justify-center">
          <AnimatePresence initial={false} custom={dir} mode="wait">
            <motion.div key={idx} custom={dir} variants={variants} initial="enter" animate="center" exit="exit" transition={{ x: { type: "spring", stiffness: 250, damping: 28 }, opacity: { duration: 0.2 } }} className="absolute w-full text-center px-4 md:px-12">
          ...
          <button onClick={() => go(-1)} className="absolute left-0 md:-left-4 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full bg-elevated/60 backdrop-blur-sm border border-border-default flex items-center justify-center text-text-muted hover:text-text-headline hover:border-border-strong transition-all z-20" aria-label="Previous"><ChevronLeft size={20} /></button>
          <button onClick={() => go(1)} className="absolute right-0 md:-right-4 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full bg-elevated/60 backdrop-blur-sm border border-border-default flex items-center justify-center text-text-muted hover:text-text-headline hover:border-border-strong transition-all z-20" aria-label="Next"><ChevronRight size={20} /></button>
        </div>
