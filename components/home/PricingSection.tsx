      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
            ...
            <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className={`flex ${plan.pop ? 'relative z-10' : 'relative'}`}>
              ...
              <div className={`relative rounded-[26px] p-8 flex flex-col w-full z-10 ${
                ...
                {plan.pop && (
                  <div className="flex items-center gap-2 mb-6 flex-wrap">
                    <div className="flex items-center gap-1.5 px-4 py-1.5 bg-copper-default/15 border border-copper-default/30 rounded-full">
                      ...
                      <span className="text-xs font-sans font-bold text-copper-default uppercase tracking-wider">Most Popular</span>
                    </div>
                    <div className="flex items-center gap-1 px-3 py-1.5 bg-turquoise-default/10 border border-turquoise-default/20 rounded-full">
                      ...
                      <span className="text-[11px] font-sans font-bold text-turquoise-default uppercase tracking-wider">Recommended</span>
                    </div>
                  </div>
                )}
                ...
                <div className="mb-8 flex items-baseline gap-2">
                  ...
                  <span className="text-sm font-sans text-text-muted">{plan.label}</span>
                </div>
                ...
                <div className="space-y-3.5 mb-10 flex-grow">
                  {plan.features.map((f, j) => (
                    <div key={j} className="text-sm font-sans text-text-body leading-snug">{f}</div>
                  ))}
                </div>

                <Link href={plan.href} className={`w-full py-4 rounded-xl font-display font-bold text-center text-base flex items-center justify-center gap-2 transition-all duration-200 ${
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className="mt-16 text-center">
          <div className="inline-flex items-center gap-2 px-5 py-2.5 bg-card border border-copper-default/20 rounded-full">
            ...
            <span className="text-sm font-sans text-text-secondary">All plans include a <span className="text-copper-default font-semibold">90-Day Money-Back Guarantee</span></span>
          </div>
        </motion.div>
      </div>
    </section>
