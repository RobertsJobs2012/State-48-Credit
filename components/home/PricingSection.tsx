        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 max-w-5xl mx-auto items-stretch">
          {plans.map((plan, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className={`flex ${plan.pop ? 'relative z-10' : 'relative'}`}>
              {plan.pop && (<><div className="absolute -inset-4 bg-copper-default/[0.08] blur-[40px] rounded-[40px] pointer-events-none" /><div className="absolute -inset-[2px] rounded-[28px] bg-gradient-to-b from-copper-default via-copper-hover/60 to-copper-default/20 z-0" /></>)}
              <div className={`relative rounded-[26px] p-8 flex flex-col w-full z-10 ${plan.pop ? 'bg-gradient-to-b from-[#201812] via-[#1A1410] to-[#141210] shadow-[0_0_60px_rgba(196,98,45,0.12)]' : 'bg-card border border-border-default hover:border-border-strong transition-all duration-300'}`}>
