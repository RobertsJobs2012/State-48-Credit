      ...
      <div className="max-w-3xl mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <span className="text-[11px] font-sans font-semibold text-copper-default uppercase tracking-[0.15em] mb-4 block">FAQ</span>
            <h2 className="text-4xl md:text-[3rem] font-display font-extrabold leading-[1.08] tracking-[-0.035em]">Frequently Asked <span className="text-copper-default">Questions</span></h2>
          </motion.div>
        </div>
            ...
            return (
              <motion.div key={i} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }}>
                <div className={`rounded-2xl border overflow-hidden transition-all duration-300 ${
                  isOpen
                    ? 'bg-gradient-to-br from-card to-elevated border-copper-default/25 shadow-[0_0_30px_rgba(196,98,45,0.06)]'
                    : 'bg-card/30 border-border-default hover:border-border-strong hover:bg-card/50'
                }`}>
                  <button onClick={() => setOpenIdx(isOpen ? null : i)} className="w-full px-7 py-6 flex items-center justify-between text-left focus:outline-none gap-4" aria-expanded={isOpen}>
                    <div className="flex items-center gap-4">
                      {/* Number badge */}
                      <span className={`text-[13px] font-display font-bold w-7 h-7 rounded-lg flex items-center justify-center shrink-0 transition-colors duration-200 ${
                        isOpen ? 'bg-copper-default/15 text-copper-default' : 'bg-white/[0.03] text-text-muted'
                      }`}>
                        {i + 1}
                      </span>
                      <span className={`text-[16px] font-display font-semibold leading-snug transition-colors duration-200 ${isOpen ? 'text-text-headline' : 'text-text-secondary'}`}>{faq.q}</span>
                    </div>
                    <motion.div
                      animate={{ rotate: isOpen ? 45 : 0 }}
                      transition={{ duration: 0.2 }}
                      className={`shrink-0 w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-200 ${
                        isOpen ? 'bg-copper-default text-background' : 'bg-white/[0.04] border border-border-default text-text-muted'
                      }`}
                    >
                  ...
                  <AnimatePresence initial={false}>
                    {isOpen && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}>
                        <div className="px-7 pb-7 pl-[4.5rem] text-[15px] font-sans text-text-secondary leading-[1.7]">{faq.a}</div>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
