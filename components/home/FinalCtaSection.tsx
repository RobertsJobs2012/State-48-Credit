    ...
    <section className="py-36 relative overflow-hidden noise-overlay dot-grid">
      ...
      <div className="max-w-4xl mx-auto px-6 relative z-10 text-center">
        <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
          <h2 className="text-[clamp(2.4rem,5.2vw,4.3rem)] font-display font-extrabold text-text-headline leading-[1.06] tracking-[-0.04em] mb-8">
            Ready to <span className="text-gradient-copper-turquoise">Rewrite</span><br />Your Financial Future?
          </h2>
          <p className="text-xl md:text-[1.3rem] text-text-lifted font-sans leading-[1.65] mb-14 max-w-2xl mx-auto">
            Stop letting past mistakes dictate your interest rates. Get your free, no-obligation credit analysis today.
          </p>
          <div className="flex flex-col sm:flex-row gap-5 justify-center items-center mb-8">
            <Link href="/get-started" className="btn-primary text-lg group w-full sm:w-auto">Start My Free Consultation <ArrowUpRight size={18} className="ml-2 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" /></Link>
            <Link href="/contact" className="btn-secondary text-lg w-full sm:w-auto">Contact Our Team</Link>
          </div>
          <div className="flex items-center justify-center gap-6 text-[11px] font-sans text-text-muted uppercase tracking-[0.1em]">
            <span className="flex items-center gap-1.5"><Shield size={12} className="text-copper-default" />No Credit Card Required</span>
