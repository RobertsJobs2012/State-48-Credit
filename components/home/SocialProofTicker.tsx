...
export function SocialProofTicker() {
  ...
  return (
    <div className="relative w-full overflow-hidden bg-sunken border-y border-border-default py-3.5">
      ...
      <div className="flex animate-[scroll_55s_linear_infinite] hover:[animation-play-state:paused]">
        {tripled.map((r, i) => (
          <div key={i} className="flex items-center gap-3 text-[13px] whitespace-nowrap px-2 shrink-0">
            <div className="flex items-center gap-0.5">
              {[1,2,3,4,5].map((s) => <Star key={s} size={10} className="text-copper-default fill-copper-default" />)}
            </div>
            <span className="text-text-lifted font-sans font-medium">&ldquo;{r.text}&rdquo;</span>
            <span className="text-text-muted font-sans">— {r.name}, {r.location}</span>
            <div className="w-1 h-1 rounded-full bg-border-strong mx-4 shrink-0" />
          </div>
        ))}
      </div>
