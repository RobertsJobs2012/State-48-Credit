...
function Counter({ to, suffix = '' }: { to: number; suffix?: string }) {
  ...
  useEffect(() => {
    ...
    const step = (ts: number) => { if (!start) start = ts; const p = Math.min((ts - start) / 2200, 1); setCount(Math.floor((1 - Math.pow(1 - p, 4)) * to)); if (p < 1) window.requestAnimationFrame(step); };
      ...
      <div className="py-24 md:py-28 bg-sunken relative noise-overlay">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-5">
            {stats.map((s, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 24, scale: 0.95 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: i * 0.1 }} className="text-center p-8 rounded-2xl bg-card/50 border border-copper-default/10 hover:border-copper-default/25 transition-all duration-300 relative group">
                ...
                <motion.div initial={{ scale: 0.8, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.1 + i * 0.1 }} className="relative">
                  <div className="text-[clamp(3rem,5.5vw,4.5rem)] font-display font-extrabold text-copper-default tracking-[-0.04em] leading-none mb-3 drop-shadow-[0_0_20px_rgba(196,98,45,0.15)]">
